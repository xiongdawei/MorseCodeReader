...
initial begin
    cnt = 1'd4;
    while (cnt >= 1'd0) begin
        if (sw[0] != prev_state[0]) begin
            morse[counter] = 1'd0;
        end
        if (sw[1] != prev_state[1]) begin
            morse[counter] = 1'd1;
        end
        if (sw[2] != prev_state[2]) begin
            morse[counter] = 1'd2;
        end
        if (sw[3] != prev_state[3]) begin
            morse[counter] = 1'd3;
        end
        counter = counter - 1;
        assign prev_state[3:0] = sw[3:0];
...
number[3] = state_eight + state_nine + state_undefined ;
number[2] = state_four + state_five + state_six + state_seven + state_undefined;
number[1] = state_two + state_three + state_six + state_seven + state_undefined;
number[0] = state_one + state_three + state_five + state_seven + state_nine;
